...
final class CarPlayBuyTicketTemplate: CarPlayTemplate {
    ...
    private var carPlayTimeOptionSelectionTemplate: CarPlayTimeOptionSelectionTemplate?
    ...
    private func bindObservers() {
        ViewStore(store, observe: { $0.templateState })
            ...
            .sink(
                receiveValue: { [weak self] in
                    ...
                    case let .validationError(_, errors):
                        self?.validationErrorTemplate(errors: errors)
                    case .preauthNewParkingProcessingSplash:
                        self?.preauthNewParkingProcessingSplash()
                    }
                })
        ...
        ViewStore(store, observe: { $0.destination })
            ...
            .sink(receiveValue: { [weak self] navigation in
                ...
                case .carSelection:
                    self?.carSelectionTemplate()
                case let .timeOptionSelection(state):
                    self?.timeOptionSelectionTemplate(state: state)
                case .zoneSelection:
                    self?.zoneSelectionTemplate()
                case let .confirmParking(state):
                    self?.confirmParkingTemplate(state: state)
                case .none:
                    guard let list = self?.list else { return }
                    self?.carPlayTimeOptionSelectionTemplate = nil
                    self?.coordinator.interfaceController?.pop(to: list, animated: true, completion: nil)
                }
            })
            .store(in: &cancellables)
    }
    ...
    private func createParkingFormSections(
        ...
        let timeRow = createSectionRow(
            ...
            handler: { [weak self] in
                self?.store.send(.onSelectTimeOptionsTap)
            }
        )
    ...
    private func accountSelectionTemplate() {
        if let store = store.scope(state: \.destination?.accountsSelection, action: \.destination.accountsSelection) {
            coordinator.append(CarPlayAccountSelectionTemplate(store: store))
        }
    }

    private func zoneSelectionTemplate() {
        if let store = store.scope(state: \.destination?.zoneSelection, action: \.destination.zoneSelection) {
            coordinator.append(CarPlayZoneSelectionTemplate(store: store))
        }
    }

    private func carSelectionTemplate() {
        if let store = store.scope(state: \.destination?.carSelection, action: \.destination.carSelection) {
            coordinator.append(CarPlayCarSelectionTemplate(store: store))
        }
    }

    private func timeOptionSelectionTemplate(state: CarPlayTimeOptionSelectionReducer.State) {
        let store = Store(initialState: state) {
            CarPlayTimeOptionSelectionReducer()
        }
        let template = CarPlayTimeOptionSelectionTemplate(store: store) { [weak self] timeOption in
            self?.store.send(.didSelectTimeOption(timeOption))
        }
        coordinator.append(template)
    }

    private func validationErrorTemplate(errors: [BuyTicketFormValidationError]) {
        let title = resourceProvider.newParkingTicketValidationErrorTitle
        let items = errors.compactMap {
            $0.toMessage(resourceProvider: resourceProvider)
        }.map {
            CPInformationItem(title: nil, detail: $0)
        }
        let action = CPTextButton(
            title: resourceProvider.genericButtonOkText,
            textStyle: .confirm,
        ...
        let tempalte = CPInformationTemplate(
            title: title,
            layout: .leading,
            items: items,
            actions: [action]
        )
        coordinator.interfaceController?.dismissAndPush(template: tempalte)
    }

    private func preauthNewParkingProcessingSplash() {
        let template = CarPlayLoadingTemplateFactory.make(message: resourceProvider.preauthNewParkingProcessingText)
        coordinator.interfaceController?.dismissAndPresent(template: template)
    }

    private func confirmParkingTemplate(state: CarPlayConfirmParkingReducer.State) {
        let store = Store(initialState: state) {
            CarPlayConfirmParkingReducer()
        }
        let template = CarPlayConfirmParkingTemplate(store: store)
        coordinator.append(template)
    }
}
