...
public struct AlternativeViewFeature: Sendable {
    ...
    public var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case let .toggleChanged(isOn):
                ...
                if !isOn {
                    disableAllItems(in: &state.items, isDisabled: true)
                } else {
                    disableAllItems(in: &state.items, isDisabled: false)
                }
                return .none
            }
        }
    ...
    private func disableAllItems(in items: inout IdentifiedArrayOf<AlternativeItemFeature.State>, isDisabled: Bool) {
        keepAncestorsAndCollapseOthers(expandedId: -1, in: &items)
        for index in items.indices {
