    ...
    public var body: some Reducer<State, Action> {
        Reduce { state, action in
            ...
            case .didLoadItems(let newItems):
                ...
                let base = IdentifiedArrayOf(uniqueElements: newItems)
                state.allItems = base
                state.items = cloneTree(base)
                return .none
            ...
            case let .searchTextChanged(text):
                ...
                guard !text.isEmpty else {
                    return .send(.didFilter(cloneTree(state.allItems)))
                }

                return performFilter(with: text, allItems: cloneTree(state.allItems))
                    .debounce(
                        id: CancelID.filter,
            ...
            case let .toggleChanged(isOn):
                state.isOn = isOn
                state.searchText = ""
                state.items = resetTreeFromAllItems(cloneTree(state.allItems), isDisabled: !isOn)
        ...
        -> IdentifiedArrayOf<AlternativeItemFeature.State> {
            ...
            IdentifiedArray(
                uniqueElements: items.map { item in
                    ...
                    item.identifiedArrayOfChildrens = reset(item.identifiedArrayOfChildrens)
                    return item
                }
            )
        }
